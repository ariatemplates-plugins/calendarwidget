{Template {
    $classpath: "light.calendar.CalendarTemplate",
    $hasScript: true,
    $res: {
    ...
    {macro renderMonth(month,first,last)}
        <table class="${skin.baseCSS}month" cellspacing="0" style="width: ${skin.skinObject.monthWidth};">
            <thead>
                <tr>
                    <th colspan="8">
                        <div class="${skin.baseCSS}monthTitle" style="position: relative;">
                            {if first && (calendar.previousPageEnabled || !settings.restrainedNavigation)}
                                <div class="previousMonth" style="position: absolute; left: 0px; top: -2px; cursor: pointer;" {on click {fn: "navigate", args : { increment: -1, incrementUnit: "M" }, scope : data.controller}/}>&laquo;</div>
                            {/if}
                            {if last && (calendar.nextPageEnabled || !settings.restrainedNavigation)}
                                <div class="nextMonth" style="position: absolute; right: 0px; top: -2px; cursor: pointer;" {on click {fn: "navigate", args : { increment: 1, incrementUnit: "M" }, scope : data.controller}/}>&raquo;</div>
                            {/if}
                            ${month.label}
                        </div>
                    </th>
    ...
    {macro renderDay(day, month)}
        ...
        {if day.monthKey==month.monthKey}
            <td ${day.isSelectable ? "data-date=\""+jsDate.getTime()+"\"":""|escapeForHTML:false}
